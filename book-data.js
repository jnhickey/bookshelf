...
// create book class to style my book, and have it listed
class Book {
  ...
  render() {
    ...
    bookInfo.innerHTML =
     `<div class="books"><h2>${this.title}</h2>
      <p><b>Author:</b> ${this.author}</p>
      <p><b>Subject:</b> ${this.subject}</p>
      <p><b>Language:</b> ${this.language}</p></div>`
    return bookInfo;
  }

};

// create my bookshelf to list out all the books in array creating my UL
class Bookshelf {
  constructor() {
    this.bookData = [];
  }
  ...
  addBook(myBook) {
    this.bookData.push(myBook);
  }

  render() {
    const myBookshelf = document.createElement("ul");
    for (let i = 0; i < this.bookData.length; i++) {
      let book = this.bookData[i];
...
// create a for loop that checks my array 
const bookshelf = new Bookshelf();
for (let i = 0; i < bookData.length; i++) {
  const bookStand = new Book(
    bookData[i].author,
    bookData[i].language,
    bookData[i].subject,
    bookData[i].title
  );
  bookshelf.addBook(bookStand);
};

// now to call on my div in html and apply render 
const bookDiv = document.body.querySelector(".bookShelf");
const bookCase = bookshelf.render();
bookDiv.appendChild(bookCase);